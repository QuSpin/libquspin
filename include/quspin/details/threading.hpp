...
namespace quspin {
  namespace details {
    ...
    concept WorkTasksInterface
        = requires(Type work_tasks, const std::size_t task_id,
        ...
        switch (schedule) {
          ...
          case Schedule::SequentialBlocks: {
            ...
            const std::size_t chunk_size
                = (num_jobs + num_threads) / num_threads;
    ...
    public:
      ...
      inline void notify_one() {
        std::atomic_flag_clear_explicit(&flag, order);
      }
      inline void wait() {
    ...
    template <std::size_t batch_size = 100, typename Iterator,
              typename Function>
    void async_for_each(Iterator &&begin, Iterator &&end, Function &&f) {
      ...
      std::vector<atomic_flag> thread_empty(
          std::thread::hardware_concurrency());
      std::vector<atomic_flag> thread_end_loop(
          std::thread::hardware_concurrency());
      ...
      for (std::size_t id = 0; id < std::thread::hardware_concurrency(); id++) {
        ...
        workers.emplace_back(worker, std::ref(thread_done.at(id)),
                             std::ref(thread_empty.at(id)),
                             std::ref(thread_end_loop.at(id)),
                             std::ref(thread_batch_n.at(id)), id);
      }

      while (begin != end) {
        for (std::size_t id = 0; id < std::thread::hardware_concurrency();
             id++) {
