...
private:
  ...
  void init_from_stl_(const std::vector<ssize_t> &shape, T *data) {
    ...
    shape_ = shape;
    ndim_ = shape.size();
    size_ = (ndim_ > 0 ? std::reduce(shape.begin(), shape.end(), 1,
...
public:
  ...
  array(const std::vector<ssize_t> &shape, T *data = nullptr) {
  ...
  const T &at(std::vector<ssize_t> &input) const {
  ...
  T &at(std::initializer_list<ssize_t> &input) {
    return at(std::vector<ssize_t>(input));
  }
};
