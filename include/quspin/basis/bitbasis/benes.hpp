...
namespace quspin::basis::benes {
  ...
  template <typename I> struct tr_bfly {
  ...
  template <typename I> struct tr_benes {
    tr_bfly<I> b1, b2;
  };
  ...
  template <typename I> I bit_permute_step(I x, I m, int shift) {
    // INLINE
  ...
  //////
  // Butterfly network
  template <typename I> I bfly(const tr_bfly<I>* self, I x) {
  ...
  template <typename I> I ibfly(const tr_bfly<I>* self, I x) {
  ...
  void gen_benes_ex(tr_benes<I>* self, const ta_index<I>& c_tgt, const ta_subword<I>& a_stage) {
    ...
    for (stage_idx = 0; stage_idx <= bit_info<I>::ld_bits - 1; ++stage_idx) {
      ...
      for (main_idx = 0; main_idx <= bit_info<I>::bits - 1;
           ++main_idx) {               // This order to meet Waksman test
  ...
  template <typename I> I benes_fwd(const tr_benes<I>* self, I x) {
  ...
  template <typename I> I benes_bwd(const tr_benes<I>* self, I x) {
    // Apply benes_perm network.
