...
namespace quspin {
  ...
  Scalar norm(const Array &arr) {
    auto kernel = [](const auto &arr) {
      ...
      norm_t result = std::accumulate(arr.begin(), arr.end(), norm_t(),
                                      [](auto &&acc, auto &&val) {
                                        return acc + details::abs_squared(val);
                                      });

      return details::ErrorOr<Scalar>(
          Scalar(details::cast<double>(std::sqrt(result))));
    };
    ...
    auto dynamic_args_check = [](const auto &arr) {
      if (arr.ndim() != 1) {
        return details::ReturnVoidError(details::Error(
            details::ErrorType::ValueError, "Expecting 1-dimensional array"));
      }
      return details::ReturnVoidError();
    };

    return details::dispatch_scalar(kernel, static_args_check,
                                    dynamic_args_check, arr);
  }
  ...
  bool allclose(const Array &lhs, const Array &rhs,
    ...
    auto kernel = [&atol, &rtol](const auto &lhs, const auto &rhs) {
      ...
      auto check = [&atol, &rtol](const auto &lhs, const auto &rhs) {
        ...
        return details::abs(l - r)
               <= atol + rtol * std::max(details::abs(l), details::abs(r));
      };
    ...
    auto static_args_check
        = [](const auto &...) { return details::ValidArgs(); };
    ...
    Scalar result = details::dispatch_scalar(kernel, static_args_check,
